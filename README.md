...
The PDF is designed on the "Marco De Vecchi CV" Design canvas on Claude, built from the same design system's CV components (CVHeader, Metric, SectionHeading, ExperienceItem, SkillTag). To update it, edit the canvas and export it from Share › Export as PDF, then replace `Marco_De_Vecchi_CV.pdf`.
